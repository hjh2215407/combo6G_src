...
public class PieChartRenderer extends DataRenderer {
    ...
    protected void drawDataSet(Canvas c, IPieDataSet dataSet) {
        ...
        for (int j = 0; j < entryCount; j++) {
            // drawNormalMark only if the value is greater than zero
        ...
        for (int j = 0; j < entryCount; j++) {
            ...
            // Don't drawNormalMark if it's highlighted, unless the chart uses rounded slices
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            for (int j = 0; j < entryCount; j++) {
                ...
                if (drawXOutside || drawYOutside) {
                    ...
                    // drawNormalMark everything, depending on settings
                    if (drawXOutside && drawYOutside) {
    ...
    protected void drawHole(Canvas c) {
        ...
        if (mChart.isDrawHoleEnabled() && mBitmapCanvas != null) {
            ...
            if (Color.alpha(mHolePaint.getColor()) > 0) {
                // drawNormalMark the hole-circle
                mBitmapCanvas.drawCircle(
                        center.x, center.y,
                        holeRadius, mHolePaint);
            }

            // only drawNormalMark the circle if it can be seen (not covered by the hole)
            if (Color.alpha(mTransparentCirclePaint.getColor()) > 0 &&
                ...
                // drawNormalMark the transparent-circle
    ...
    public void drawHighlighted(Canvas c, ArrayList<Highlight> indices) {
        ...
        for (int i = 0; i < indices.size(); i++) {
            ...
            int index = (int) indices.get(i).getX();

            if (index >= drawAngles.length)
                continue;

            IPieDataSet set = mChart.getData()
                    .getDataSetByIndex(indices.get(i)
                            .getDataSetIndex());
