...
//        dragHighlight = mChart.getHighlightByTouchPoint(e.getX(), e.getY());
        ...
        if(mLastHighlighted.size() <= mSelectedIndex) {
            Log.d("performHighlightDrag", "mLastHighlighted size is 0");
            return;
        }

        if (mLastHighlighted.get(mSelectedIndex) == null || mLastHighlighted.get(mSelectedIndex).isFixed())
            return;

        if (mSelectedIndex != -1 && e != null && mLastHighlighted != null &&
                mChart.getEntryByTouchPoint(e.getX(), e.getY()) != null &&
                mLastHighlighted.get(mSelectedIndex) != null) {

            Entry touchingEntry = mChart.getEntryByTouchPoint(e.getX(), e.getY());
            if(touchingEntry == null) return;

//            Log.d("MarkerDragEvent", "touching entry x : " + touchingEntry.getX());

            float x = touchingEntry.getX();
            float y = 0;

            if (mChart.getData().getDataSetByIndex(mLastHighlighted.get(mSelectedIndex).getDataSetIndex()).getEntriesForXValue(x).size() != 0) {

                y = mChart.getData().getDataSetByIndex(mLastHighlighted.get(mSelectedIndex).getDataSetIndex()).getEntryForXValue(x, touchingEntry.getY()).getY();

            } else return;

            int dataSetIndex = mLastHighlighted.get(mSelectedIndex).getDataSetIndex();

            dragHighlight = mLastHighlighted.get(mSelectedIndex).getCopyHighlight(x, y, dataSetIndex);
            dragHighlight.setDraw(e.getX(), e.getY());
//            if(dragHighlight.getY() == 0f) return;
        ...
        if (dragHighlight != null && mSelectedIndex != -1 && mLastHighlighted.get(mSelectedIndex) != null &&
                ...
                mChart.getXAxis().getAxisMaximum() >= mLastHighlighted.get(mSelectedIndex).getX() &&
                mChart.getXAxis().getAxisMinimum() <= mLastHighlighted.get(mSelectedIndex).getX() &&
                min < touchDist) {

            addLastHighlighted(mSelectedIndex, dragHighlight);
            mChart.highlightValue(dragHighlight, mSelectedIndex, true);
//            Log.d("getDataIndex", "Selected Data Index : " + mSelectedIndex);
//            Log.d("minDistance", min + "");
//            Log.d("MarkerDragEvent", "dragHighlight : " + dragHighlight.getX() + " " + dragHighlight.getY() + " " + dragHighlight.getDrawX() + " " + dragHighlight.getDrawY());

        }

    }

    private void performHighlightDown(MotionEvent e) {
        if (mLastHighlighted.size() == 0) {
            Log.d("performHighlightDown", "size 0");
            mLastHighlighted.add(dragHighlight);
        }
//        if(mLastHighlighted.get(0) == null) {
//            Log.d("performHighlightDown", "index 0 null");
//            return;
//        }
...
//            Log.d("performHighlightDrag", "Event getX() : " + e.getX());
            ...
            //org
            /*if (Math.abs(mLastHighlighted.get(i).getDrawX() - e.getX()) < min) {
                min = Math.abs(mLastHighlighted.get(i).getDrawX() - e.getX());

                minDistanceIndex = i;

            } else if(Math.abs(mLastHighlighted.get(i).getDrawX() - e.getX()) <= min && mSelectedIndex == i) {
                minDistanceIndex = i;
            }*/
            //org

            //@@ [21.12.21] Marker 이동 관련 겹쳐져 있을 때 fixed marker 는 제외하고 normal 로 되어 있는 marker 선택
            if (Math.abs(mLastHighlighted.get(i).getDrawX() - e.getX()) < min) {
                min = Math.abs(mLastHighlighted.get(i).getDrawX() - e.getX());

                minDistanceIndex = i;

            } else if(Math.abs(mLastHighlighted.get(i).getDrawX() - e.getX()) == min) {
                if (mLastHighlighted.get(i).isFixed()) continue;
                else minDistanceIndex = i;
            } else if(Math.abs(mLastHighlighted.get(i).getDrawX() - e.getX()) < min && mSelectedIndex == i) {
                minDistanceIndex = i;
            }
            //@@
        ...
        } else mSelectedIndex = minDistanceIndex;

        Log.d("performHighlightDrag", "Min Index : " + minDistanceIndex);
    }
    ...
    public boolean onSingleTapUp(MotionEvent e) {
        ...
        if (mSelectedIndex != -1 && mLastHighlighted.get(mSelectedIndex) != null &&
                mLastHighlighted.get(mSelectedIndex).getMarkerType().equals("R") &&
                mChart.getData().getEntryCount() != 0 &&
                h != null) {
            h.setMarkerTypeToRef();
        } else if (mSelectedIndex != -1 && mChart.getData().getEntryCount() != 0 && h != null) {
            h.setMarkerTypeToDelta();
        }

        if (mSelectedIndex != -1 && mLastHighlighted.get(mSelectedIndex) != null &&
                mLastHighlighted.get(mSelectedIndex).isContinuous() &&
                mChart.getData().getEntryCount() != 0 &&
                h != null) {
