...
public abstract class DataSet<T extends Entry> extends BaseDataSet<T> {
    ...
    public T getEntryForXValue(float xValue, float closestToY, Rounding rounding) {
        ...
        try {
            ...
            if (index > -1 && index < mValues.size() && mValues.get(index) != null)
    ...
    public T getEntryForIndex(int index) {
        ...
        if (mValues.size() <= index || index == -1) {
            Log.d("getEntryForIndex", "out of index..." + index);
            return null;
        }
    ...
    public int getEntryIndex(float xValue, float closestToY, Rounding rounding) {
        ...
        while (low < high) {
            ...
            if((m+1) >= mValues.size()) {
                Log.d("getEntryIndex", "out of index...");
                break;
            }

            Entry d1Entry = mValues.get(m);
            Entry d2Entry = mValues.get(m+1);

            if(d1Entry == null || d2Entry == null) {
                Log.d("getEntryIndex", "entry is null.. break");
                break;
            }

            final float d1 = d1Entry.getX() - xValue,
                    d2 = d2Entry.getX() - xValue,
        ...
        if (closest != -1) {
            ...
            if(closest >= mValues.size()) return -1;

            Entry closestEntry = mValues.get(closest);
            if(closestEntry == null) {
                Log.d("getEntryIndex", "closest entry is null ... return -1");
                return -1;
            }
    ...
    public List<T> getEntriesForXValue(float xValue) {
        ...
        while (low <= high) {
            ...
            if (entry == null) {
                Log.d("getEntriesForXValue", "entry is null..");
                return null;
            }
