...
//            }
//
//            for (int i = 8; i < lineData.getDataSets().size(); i++) {
//
//                ILineDataSet set = lineData.getDataSets().get(i);
//                if (set.isVisible())
//                    drawDataSet(c, set);
...
//        }
    ...
    protected void drawCubicBezier(ILineDataSet dataSet) {
        ...
        if (mXBounds.range >= 1) {
            ...
            for (int j = mXBounds.min + 1; j <= mXBounds.range + mXBounds.min; j++) {
                ...
                if(prevPrev == null || prev == null || cur == null) {
                    Log.d("drawCubicBezier", "entry is null");
                    return;
                }
                ...
                if(next == null) {
                    Log.d("drawCubicBezier", "next entry is null");
                    return;
                }
    ...
    protected void drawCubicFill(Canvas c, ILineDataSet dataSet, Path spline, Transformer trans, XBounds bounds) {
        ...
        try {
            ...
            if (dataSet.getEntryForIndex(bounds.min + bounds.range) == null || dataSet.getEntryForIndex(bounds.min) == null) return;
    ...
    protected void drawLinear(Canvas c, ILineDataSet dataSet) {
        ...
        // if the data-set is dashed, drawNormalMark on bitmap-canvas
...
//        }
    ...
    @Override
    public void drawHighlighted(Canvas c, ArrayList<Highlight> indices) {

        LineData lineData = mChart.getLineData();

        for (Highlight high : indices) {
//            Log.d("LineChartRenderer", "Min : " + lineData.getXMin());
//            Log.d("LineChartRenderer", "Max : " + lineData.getXMax());

            if (high == null || high.getX() < lineData.getXMin() || high.getX() > lineData.getXMax())
                continue;

            ILineDataSet set;

            try {
                set = lineData.getDataSetByIndex(high.getDataSetIndex());
            } catch (NullPointerException e) {
                e.printStackTrace();
            ...
            MPPointD pix = mChart.getTransformer(set.getAxisDependency()).getPixelForValues(high.getX(), high.getY() * mAnimator
                    .getPhaseY());

            high.setDraw((float) pix.x, (float) pix.y);
//            Log.d("high", high.getY() + "");
//            Log.d("Highlight", e.getY() + "");

            // drawNormalMark the lines
            drawHighlightLines(c, (float) pix.x, (float) pix.y, set);
        }
    }

    public void drawSelectedHighlight(Canvas c, Highlight high) {
        ...
        if (e != null)
            pix = mChart.getTransformer(set.getAxisDependency()).getPixelForValues(high.getX(), high.getY() * mAnimator
                    .getPhaseY());
        else {
            pix = mChart.getTransformer(set.getAxisDependency()).getPixelForValues(0, 0 * mAnimator
                    .getPhaseY());
        }
...
//        Log.d("Highlight", e.getY() + "");
        ...
        // drawNormalMark the lines
        drawSelectedHighlightLines(c, (float) pix.x, (float) pix.y, set);
    }
