...
public abstract class ChartData<T extends IDataSet<? extends Entry>> {
    ...
    public Entry getEntryForHighlight(Highlight highlight) {
        if (highlight != null && highlight.getDataSetIndex() != -1 && highlight.getDataSetIndex() >= mDataSets.size())
            return null;
        else if(highlight != null && highlight.getDataSetIndex() != -1){
    ...
    public void setDataSet(int idx, T d) {
    ...
    public void clearValues(int idx) {
        if (mDataSets != null && mDataSets.size() > idx) {
            mDataSets.get(idx).clear();
        }
        notifyDataChanged();
    }

    public void clearValuesNotChanging(int idx) {
        if (mDataSets != null && mDataSets.size() > idx) {
            mDataSets.get(idx).clear();
        }
    }
