...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends
    ...
    protected void init() {
        ...
        for (int i = 0; i < MAX_MARKER_SIZE; i++) {
            mChartTouchListener.addLastHighlighted(null);
        }
...
//        Log.d("valuesToHighlihght", valuesToHighlight()  + "");
            if (valuesToHighlight()) {

                try {
//            Log.d("mRenderer", "drawHighlight");
                    mRenderer.drawHighlighted(canvas, mChartTouchListener.getLastHighlighted());
                    if (mChartTouchListener.getLastHighlighted() != null && mChartTouchListener.getLastHighlighted().size() > mChartTouchListener.getSelectedHighlightIndex()
                            && mChartTouchListener.getSelectedHighlightIndex() != -1 && mData.getEntryCount() > 0) {
                        mRenderer.drawSelectedHighlight(canvas, (Highlight) mChartTouchListener.getLastHighlighted().get(mChartTouchListener.getSelectedHighlightIndex()));
    ...
    protected void drawGridBackground(Canvas c) {
        ...
        if (mDrawGridBackground) {
            ...
            // drawNormalMark the grid background
    ...
    /**
     * set this to true to drawNormalMark the grid background, false if not
     *
     * @param enabled
     */
