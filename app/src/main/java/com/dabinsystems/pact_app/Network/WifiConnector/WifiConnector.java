package com.dabinsystems.pact_app.Network.WifiConnector;
...
import com.dabinsystems.pact_app.Network.interfaces.RemoveWifiListener;
import com.dabinsystems.pact_app.Network.interfaces.ShowWifiListener;
import com.dabinsystems.pact_app.Network.interfaces.WifiStateListener;
...
public class WifiConnector {
    ...
    public void setWifiConfiguration(String SSID, String BSSID, String securityType, String password) {
        ...
        this.wifiConfiguration.SSID = convertToQuotedString(SSID);
        this.wifiConfiguration.BSSID = BSSID;
        this.wifiConfiguration.priority = 40;
...
//            wifiConfiguration.allowedGroupCiphers.set(WifiConfiguration.GroupCipher.CCMP);
    ...
    public void connectWifi2() {
        wifiLog("connectWifi2 ssid = " + mSSID + ", pw = " + mPW);

        WifiNetworkSpecifier wifiNetworkSpecifier = new WifiNetworkSpecifier.Builder()
                .setSsid(mSSID)// wifiConfiguration.SSID) //SSID 이름
                .setWpa2Passphrase(mPW)//wifiConfiguration.preSharedKey) //비밀번호, 보안설정 WPA2
                .build();
        ...
        connectivityManager.requestNetwork(networkRequest, new ConnectivityManager.NetworkCallback() {
            ...
            public void onAvailable(@NonNull Network network) {
                ...
                if (logOrNot) Log.d(TAG, "onAvailable ");

                if (connectionResultListener != null)
                    connectionResultListener.successfulConnect(wifiConfiguration.SSID);
            }

            @Override
            public void onUnavailable() {
                super.onUnavailable();
                if (logOrNot) Log.d(TAG, "onUnavailable ");
            ...
            public void onLosing(@NonNull Network network, int maxMsToLive) {
                ...
                if (logOrNot) Log.d(TAG, "onLosing ");
            }

            @Override
            public void onLost(@NonNull Network network) {
                super.onLost(network);
                if (logOrNot) Log.d(TAG, "onLost ");
            }

            @Override
            public void onCapabilitiesChanged(@NonNull Network network, @NonNull NetworkCapabilities networkCapabilities) {
                super.onCapabilitiesChanged(network, networkCapabilities);
                if (logOrNot) Log.d(TAG, "onCapabilitiesChanged ");
            }

            @Override
            public void onLinkPropertiesChanged(@NonNull Network network, @NonNull LinkProperties linkProperties) {
                super.onLinkPropertiesChanged(network, linkProperties);
                if (logOrNot) Log.d(TAG, "onLinkPropertiesChanged ");
            }

            @Override
            public void onBlockedStatusChanged(@NonNull Network network, boolean blocked) {
                super.onBlockedStatusChanged(network, blocked);
                if (logOrNot) Log.d(TAG, "onBlockedStatusChanged blocked = " + blocked);
            }
        });
    }
    ...
    private boolean connectToWifiAccesPoint() {
        ...
        wifiLog("ssid : " + wifiConfiguration.SSID);
        wifiLog("network id found: " + networkId);
        if (networkId == -1) {
            if (wifiConfiguration == null) wifiLog("wifiConfiguration is null");
    ...
    private int getNetworkId(String SSID) {
        ...
        if (confList != null && confList.size() > 0) {
            wifiLog("confList is not null");
            for (WifiConfiguration existingConfig : confList) {

                wifiLog("confList : " + existingConfig.SSID + " id : " + existingConfig.networkId);
    ...
    private void wifiLog(String text) {
        //if (logOrNot) InitActivity.logMsg(TAG, "WifiConnector: " + text);
        if (logOrNot) Log.d(TAG, text);
    }
