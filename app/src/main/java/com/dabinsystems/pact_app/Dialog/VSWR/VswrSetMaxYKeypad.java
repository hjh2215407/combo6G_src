...
public class VswrSetMaxYKeypad extends CustomBaseDialog {
    ...
    public VswrSetMaxYKeypad(MainActivity activity, ActivityMainBinding binding) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (FunctionHandler.getInstance().getMeasureType().getType() == MeasureType.MEASURE_TYPE.DTF_RL
                || FunctionHandler.getInstance().getMeasureType().getType() == MeasureType.MEASURE_TYPE.RL
                || FunctionHandler.getInstance().getMeasureType().getType() == MeasureType.MEASURE_TYPE.CABLE_LOSS)
            mKeypadBinding.tvdB.setVisibility(View.VISIBLE);
        else mKeypadBinding.tvdB.setVisibility(View.INVISIBLE);
            ...
            try {
                ...
                switch (type) {
                    ...
                    case DTF_VSWR:
                        ...
                        if (val <= VswrAmplitudeData.DEFALUT_VSWR_Y_MAX
                                && val > VswrDataHandler.getInstance().getConfigData().getAmplitudeData().getAmpMin()
                                && val > VswrAmplitudeData.DEFALUT_VSWR_Y_MIN) {

                            VswrDataHandler.getInstance().getConfigData().getAmplitudeData().setAmpMax(val);
                            FunctionHandler.getInstance().getMainLineChart().setMaxYRange(val);
                            FunctionHandler.getInstance().getMainLineChart().invalidate();

                        } else {
                            Toast.makeText(mActivity, "Out of range (1 ~ 65)", Toast.LENGTH_SHORT).show();
                        }
                    ...
                    case CABLE_LOSS:
                        ...
                        if (val <= VswrAmplitudeData.DEFALUT_RL_Y_MAX && val > VswrAmplitudeData.DEFALUT_RL_Y_MIN
                                && val > VswrDataHandler.getInstance().getConfigData().getAmplitudeData().getAmpMin()) {
                            ...
                            Toast.makeText(mActivity, "Out of range (0 ~ 60)", Toast.LENGTH_SHORT).show();
