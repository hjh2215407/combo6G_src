...
public class Nr5gConstellationKeypadDialog extends CustomBaseDialog {
    ...
    public Nr5gConstellationKeypadDialog(MainActivity activity, ActivityMainBinding binding) {
    ...
    private void clickEnter() {
        ...
        try {
            if (mKeypadBinding.tvView.getText().length() == 0) {
                ...
                FunctionHandler.getInstance().getScatterChart().setConstellationScale(val);

                mMainBinding.demodulationLayout.scatterChart.invalidate();
                mMainBinding.demodulationLayout.scatterChart.notifyDataSetChanged();

                ViewHandler.getInstance().getNrMeasSetupView2().update();
