...
import com.dabinsystems.pact_app.databinding.AttenKeypadBinding;

import static com.dabinsystems.pact_app.View.DynamicView.convertDpToPixel;

public class LteFddAttenuatorKeypadDialog extends CustomBaseDialog {

    private AttenKeypadBinding mKeypadBinding;
    ...
    public LteFddAttenuatorKeypadDialog(MainActivity activity, ActivityMainBinding binding) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mKeypadBinding = DataBindingUtil.inflate(LayoutInflater.from(mActivity), R.layout.atten_keypad, null, false);
        setContentView(mKeypadBinding.getRoot());

        if (FunctionHandler.getInstance().getMeasureMode().getMode() == MeasureMode.MEASURE_MODE.VSWR) mKeypadBinding.tvdB.setVisibility(View.INVISIBLE);
        else if(FunctionHandler.getInstance().getMeasureMode().getMode() == MeasureMode.MEASURE_MODE.DTF) mKeypadBinding.tvdB.setVisibility(View.VISIBLE);
    ...
    private void clickEnter() {
        ...
        try {
            if (mKeypadBinding.tvView.getText().length() == 0) {
                ...
                DataHandler.getInstance().getLteFddData().getAmpData().setAttenuator(val);
