...
public class RefLevKeypadDialog extends CustomBaseDialog {
    ...
    public RefLevKeypadDialog(MainActivity activity, ActivityMainBinding binding) {
    ...
    private void clickEnter() {
        ...
        try {
            if (mKeypadBinding.tvView.getText().length() == 0) {
                ...
                FunctionHandler.getInstance().getMainLineChart().setRefLev(val);
                FunctionHandler.getInstance().getGateLineChart().setRefLev(val);

                SaDataHandler.getInstance().getConfigData().getAmplitudeData().setRefLev(val);
