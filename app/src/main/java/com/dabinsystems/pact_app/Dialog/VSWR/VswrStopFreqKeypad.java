...
public class VswrStopFreqKeypad extends CustomBaseDialog {
    ...
    public VswrStopFreqKeypad(MainActivity activity, ActivityMainBinding binding) {
    ...
    private void clickEnter() {
        ...
        mActivity.runOnUiThread(()->{
            ...
            try {
                ...
                if (val <= VswrDataHandler.getInstance().getConfigData().getFrequencyData().getStartFreq()) {
                    Toast.makeText(mActivity, "Must be greater than Start Freq.", Toast.LENGTH_SHORT).show();
                }
                else if (val >= VswrFrequencyData.DEFALUT_MIN_FREQ && val <= VswrFrequencyData.DEFALUT_MAX_FREQ) {

                    VswrDataHandler.getInstance().getConfigData().getFrequencyData().setStopFreq(val);
                    ViewHandler.getInstance().getFrequencyView().update();
                    if(!VswrDataHandler.getInstance().getConfigData().checkDistance())
                    ...
                    Toast.makeText(mActivity, "Out of range(" + VswrFrequencyData.DEFALUT_MIN_FREQ + "MHz ~ " + VswrFrequencyData.DEFALUT_MAX_FREQ + "MHz)", Toast.LENGTH_SHORT).show();
                }
