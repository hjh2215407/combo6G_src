...
public class SemSpanFreqKeypad extends CustomBaseDialog {
    ...
    public SemSpanFreqKeypad(MainActivity activity, ActivityMainBinding binding) {
    ...
    private void clickEnter() {
        ...
        try {
            if (mKeypadBinding.tvView.getText().length() == 0) {
                ...
                SaDataHandler.getInstance().getSemConfigData().getSemMeasSetupData().getRefChannelData().setSpan(val);
//                FunctionHandler.getInstance().getMainLineChart().setSpan(val);
                ViewHandler.getInstance().getRefChannelView().update();
