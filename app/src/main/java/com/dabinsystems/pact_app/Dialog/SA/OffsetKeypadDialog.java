...
public class OffsetKeypadDialog extends CustomBaseDialog {
    ...
    public OffsetKeypadDialog(MainActivity activity, ActivityMainBinding binding) {
    ...
    private void clickEnter() {
        ...
        try {
            if (mKeypadBinding.tvView.getText().length() == 0) {
                ...
                FunctionHandler.getInstance().getMainLineChart().setOffset(val);
                FunctionHandler.getInstance().getGateLineChart().setOffset(val);

                SaDataHandler.getInstance().getConfigData().getAmplitudeData().setOffset(val);
