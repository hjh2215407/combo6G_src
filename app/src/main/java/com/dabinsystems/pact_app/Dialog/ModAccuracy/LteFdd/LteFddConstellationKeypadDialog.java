...
public class LteFddConstellationKeypadDialog extends CustomBaseDialog {
    ...
    public LteFddConstellationKeypadDialog(MainActivity activity, ActivityMainBinding binding) {
    ...
    private void clickEnter() {
        ...
        try {
            if (mKeypadBinding.tvView.getText().length() == 0) {
                ...
                FunctionHandler.getInstance().getScatterChart2().setConstellationScale(val);

                mMainBinding.lteFddLayout.scatterChart.invalidate();
                mMainBinding.lteFddLayout.scatterChart.notifyDataSetChanged();

                ViewHandler.getInstance().getLteFddMeasSetupView().update();
