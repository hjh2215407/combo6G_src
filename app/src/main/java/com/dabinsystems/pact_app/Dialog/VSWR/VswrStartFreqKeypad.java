...
public class VswrStartFreqKeypad extends CustomBaseDialog {
    ...
    public VswrStartFreqKeypad(MainActivity activity, ActivityMainBinding binding) {
    ...
    private void clickEnter() {
        ...
        mActivity.runOnUiThread(() -> {
            ...
            try {
                ...
                if (val >= VswrDataHandler.getInstance().getConfigData().getFrequencyData().getStopFreq()) {
                    Toast.makeText(mActivity, "Must be less than Stop Freq.", Toast.LENGTH_SHORT).show();
                }
                else if (val >= VswrFrequencyData.DEFALUT_MIN_FREQ && val <= VswrFrequencyData.DEFALUT_MAX_FREQ) {

                    VswrDataHandler.getInstance().getConfigData().getFrequencyData().setStartFreq(val);
