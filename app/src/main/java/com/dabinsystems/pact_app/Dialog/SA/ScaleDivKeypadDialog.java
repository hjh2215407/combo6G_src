...
public class ScaleDivKeypadDialog extends CustomBaseDialog {
    ...
    public ScaleDivKeypadDialog(MainActivity activity, ActivityMainBinding binding) {
    ...
    private void clickEnter() {
        ...
        try {
            if (mKeypadBinding.tvView.getText().length() == 0) {
                ...
                if(val < 0.1f || val > 20f) return;
                FunctionHandler.getInstance().getMainLineChart().setScaleDiv(val);
                FunctionHandler.getInstance().getMainLineChart().updateAclrBox();
                FunctionHandler.getInstance().getMainLineChart().invalidate();
                SaDataHandler.getInstance().getConfigData().getAmplitudeData().setScaleDiv(val);
