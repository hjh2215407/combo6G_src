...
public class Nr5gFreqOfCarrierKeypad extends CustomBaseDialog {
    ...
    public Nr5gFreqOfCarrierKeypad(MainActivity activity, ActivityMainBinding binding) {
    ...
    public Nr5gFreqOfCarrierKeypad(MainActivity activity, ActivityMainBinding binding, int idx) {
        super(activity, R.style.AppFullScreenTheme);
    ...
    private void clickEnter() {
        ...
        try {
            if (mKeypadBinding.tvView.getText().length() == 0) {
                ...
                if(DataHandler.getInstance().getNrData().getInterTaeData().setFreqOfCarrier(idx, val)) {

                    ViewHandler.getInstance().getInterTaeView().update();
