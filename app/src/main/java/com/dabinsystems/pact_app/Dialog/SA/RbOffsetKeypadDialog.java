...
public class RbOffsetKeypadDialog extends CustomBaseDialog {
    ...
    public RbOffsetKeypadDialog(MainActivity activity, ActivityMainBinding binding) {
    ...
    private void clickEnter() {
        ...
        try {
            if (mKeypadBinding.tvView.getText().length() == 0) {
                ...
                DataHandler.getInstance().getNrData().getSsbInfoData().setRbOffset(val);

                FunctionHandler.getInstance().getDataConnector().sendCommand(
                        DataHandler.getInstance().getNrData().getNrCmd()
                );

                ViewHandler.getInstance().getSSBInfoView().update();
